...
def test_short_product_names():
    ...
    Владелец задавал спеццены списком («Бутастим 100 мл 420») — короткие
    имена не дотягивали до порога, бот отвечал «ни один товар не распознан».
    """
    _fresh_db()
    # весь список того заказа — должен распознаться целиком
    order = [("Бутастим", "100 мл", 57), ("Мульти-Витатоп", "50 мл", 22),
             ("Мульти-Витатоп", "100 мл", 23), ("Бутатоп", "50 мл", 14),
             ("Клозатоп", "100 мл", 18), ("Клозатоп", "1 л", 20),
             ("Пенстоп-G", "50 мл", 27), ("Элеовит", "100 мл", 62),
             ("Альбен плюс 100", "100 мл", 72), ("Албенивер", "1 л", 79),
             ("Тетравитам", "100 мл", 63), ("Топмектин гель", "30 мл", 44),
             ("Топфлунекс", "100 мл", 49), ("Празимектоп", "500 мл", 31),
             ("Окситоп", "100 мл", 26), ("Топзантел", "100 мл", 41),
             ("Клозан плюс 100", "100 мл", 89)]
    for name, volume, pid in order:
        got = prices.match_product(name, volume)
        assert got is not None and got["id"] == pid, f"{name} {volume} -> {got}"
    # опечатка в коротком имени тоже проходит
    assert prices.match_product("Альбенивер", "1 л")["id"] == 79
    # фасовка без пробела
    assert prices.match_product("Бутастим", "100мл")["id"] == 57
    # ...но лишнего не выдумываем
    assert prices.match_product("Клозатоп", "100 таб") is None   # это Клозатоп-Форте
    assert prices.match_product("Ерунда", "100 мл") is None
    # близкие имена не путаются между собой
    assert prices.match_product("Топмектин", "100 мл")["id"] == 37
    assert prices.match_product("Топмектин гель", "30 мл")["id"] == 44
    assert prices.match_product("Бутатоп", "100 мл")["id"] == 15      # не Бутастим
    assert prices.match_product("Бутастим", "100 мл")["id"] == 57     # не Бутатоп
